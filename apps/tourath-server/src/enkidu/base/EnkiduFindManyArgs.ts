/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EnkiduWhereInput } from "./EnkiduWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { EnkiduOrderByInput } from "./EnkiduOrderByInput";

@ArgsType()
class EnkiduFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => EnkiduWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => EnkiduWhereInput, { nullable: true })
  @Type(() => EnkiduWhereInput)
  where?: EnkiduWhereInput;

  @ApiProperty({
    required: false,
    type: [EnkiduOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [EnkiduOrderByInput], { nullable: true })
  @Type(() => EnkiduOrderByInput)
  orderBy?: Array<EnkiduOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { EnkiduFindManyArgs as EnkiduFindManyArgs };
